<div class="mx-auto max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-6 sm:p-8">
    ...
    <p class="mt-1 text-sm text-slate-400">Acesse o painel, as preferências e os alertas.</p>
    <form wire:submit="login" class="mt-6 space-y-4">
        ...
        <div>
            ...
            <input type="password" wire:model="password" autocomplete="current-password" class="w-full rounded-lg border border-slate-700 bg-slate-950 px-4 py-3">
            @error('password') <p class="mt-1 text-sm text-red-400">{{ $message }}</p> @enderror
        </div>
        <label class="flex items-center gap-3 text-sm text-slate-300">
            <input type="checkbox" wire:model="remember" class="h-4 w-4 shrink-0 rounded border-slate-600 accent-emerald-500">
