...
from shared.models import FipeMatchResult, LotRecord, PalacioLotRaw
...
def run_collector() -> dict[str, Any]:
    ...
    with PalacioClient() as palacio, FipeClient() as fipe:
        ...
        for raw in palacio.iter_open_vehicle_lots():
            try:
                previous = repo.get_lot(f"palacio:{raw.lot_id}")
                ...
                errors += 1
                logger.exception("Failed to process Palácio lot %s", getattr(raw, "lot_id", "?"))
                continue
            processed += 1
